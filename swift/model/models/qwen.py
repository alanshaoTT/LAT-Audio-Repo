...
def _compat_qwen3_omni_mixed_data(model, processor):
    print("hereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
    # print(hasattr(model, 'origin_forward'))
    if hasattr(model, 'origin_forward'):
        return
    ...
    def forward(
        ...
        **kwargs,
        ...
        # import pdb; pdb.set_trace()
        # AIGC START
        # 环境变量开关：只有显式 True 才强制一直用这个 forward
        use_wrapper = get_env_args('QWEN3_OMNI_USE_SWIFT_FORWARD', bool, None)
        # use_wrapper 不是 True 的情况下，保持原来的逻辑：
        # 非训练 或 未启用 Deepspeed -> 走 transformers 自带的 origin_forward
        # if use_wrapper is not True and (not self.training or not is_deepspeed_enabled()):
        if use_wrapper is not True and (not is_deepspeed_enabled()):
            # 不把自定义的长度信息传给 transformers 原生 forward
            kwargs_origin = {
                k: v
                for k, v in kwargs.items()
                if k not in ('first_audio_token_len', 'first_audio_token_len_orig')
            }
            return self.origin_forward(
                input_ids=input_ids,
        ...
        # 当 QWEN3_OMNI_USE_SWIFT_FORWARD=true 时，不会进入上面的 return，
        # 无论是不是训练 / Deepspeed，都会继续走下面这套 swift 的 forward。
        # AIGC END
    ...
    model.origin_forward = model.forward
    model.forward = MethodType(forward, model)
    print("use modefied forward!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    if hasattr(model, "_validate_model_kwargs") and not hasattr(model, "origin_validate_model_kwargs"):
        model.origin_validate_model_kwargs = model._validate_model_kwargs
        def _validate_model_kwargs_patched(self, model_kwargs):
            # 注意：这是 generate 传进来的 copy()，删这里不会影响真正传给 forward 的 kwargs
            for k in ("first_audio_token_len", "first_audio_token_len_orig", "audio_token_lens_orig_list"):
                model_kwargs.pop(k, None)
            return self.origin_validate_model_kwargs(model_kwargs)
        model._validate_model_kwargs = MethodType(_validate_model_kwargs_patched, model)
