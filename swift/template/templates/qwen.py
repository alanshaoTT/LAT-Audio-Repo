...
class Qwen2_5OmniTemplate(Qwen2_5VLTemplate):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        if feature_attention_mask is not None:
            ...
            #// AIGC START
            # 根据环境变量压缩第一个 mp3 的帧数，并记录：
            # 1）当前样本所有 mp3 的原始 token 长度列表
            # 2）当前样本第一个 mp3 压缩前 / 压缩后的 token 数
            factor = get_env_args('AUDIO_SEQ_COMP_FACTOR', int, 1)
            # 原始每个 mp3 的 token 数（未压缩），形状：[num_audios_this_sample]
            audio_token_lens_orig = self._get_feat_extract_output_lengths(audio_feature_lengths)
            first_audio_token_len_orig = None
            first_audio_token_len = None
            audio_token_lens_orig_list = None
            audio_lengths = None  # 兼容后面使用 audio_lengths 的逻辑
            if audio_token_lens_orig is not None and audio_token_lens_orig.numel() > 0:
                # 当前样本所有 mp3 的原始 token 长度列表（Python list，后面 collator 会按样本收集）
                audio_token_lens_orig_list = [int(x.item()) for x in audio_token_lens_orig]
                # 当前样本第一个 mp3 的原始 token 数
                first_audio_token_len_orig = audio_token_lens_orig_list[0]
            # 只动当前样本的第一个 mp3，对应的 feature 长度做 2/4/8 倍下采样
            if (
                factor in (2, 4, 8)
                and audio_feature_lengths is not None
                and audio_feature_lengths.numel() > 0
            ):
                audio_feature_lengths = audio_feature_lengths.clone()
                audio_feature_lengths[0] = torch.clamp(
                    audio_feature_lengths[0] // factor,
                    min=1,
                )
            # 压缩后再算一次 token 数，从而拿到第一个 mp3 的压缩后 token 数
            audio_token_lens_after = self._get_feat_extract_output_lengths(audio_feature_lengths)
            if audio_token_lens_after is not None and audio_token_lens_after.numel() > 0:
                # print("template中压缩了！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！")
                first_audio_token_len = int(audio_token_lens_after[0].item())
                audio_lengths = audio_token_lens_after
            #// AIGC END
        else:
            audio_lengths = None
        ...
        #// AIGC START
        # 把第一个 mp3 的压缩前/压缩后 token 数，以及当前样本所有 mp3 的原始长度列表带出去
        if 'audio_feature_lengths' in locals():
            if 'audio_token_lens_orig_list' in locals() and audio_token_lens_orig_list is not None:
                # list[int]，这个样本里每个 mp3 的原始 token 数
                encoded['audio_token_lens_orig_list'] = audio_token_lens_orig_list
            if 'first_audio_token_len_orig' in locals() and first_audio_token_len_orig is not None:
                encoded['first_audio_token_len_orig'] = first_audio_token_len_orig
            if 'first_audio_token_len' in locals() and first_audio_token_len is not None:
                encoded['first_audio_token_len'] = first_audio_token_len
        #// AIGC END
        encoded.update(media_inputs)
        return encoded
    ...
    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        #// AIGC START
        # 收集：
        # 1）每个样本第一个 mp3 的压缩前 / 压缩后 token 数（[B]）
        # 2）每个样本所有 mp3 的原始 token 长度列表（list of list）
        # 转为 Python int，避免 batch 里已是 tensor 时 torch.tensor(list_of_tensors) 报错
        def _to_int(x):
            return int(x.item()) if isinstance(x, torch.Tensor) else int(x)

        first_audio_token_lens = [_to_int(b.get('first_audio_token_len', 0)) for b in batch]
        first_audio_token_lens_orig = [_to_int(b.get('first_audio_token_len_orig', 0)) for b in batch]
        res['first_audio_token_len'] = torch.tensor(first_audio_token_lens, dtype=torch.long)
        res['first_audio_token_len_orig'] = torch.tensor(first_audio_token_lens_orig, dtype=torch.long)
        # list-of-lists：len = B，每个元素是该样本所有 mp3 的原始 token 数列表
        res['audio_token_lens_orig_list'] = [
            b.get('audio_token_lens_orig_list', []) for b in batch
        ]
        #// AIGC END
        input_features = [b['input_features'] for b in batch if b.get('input_features') is not None]
        feature_attention_mask = [
